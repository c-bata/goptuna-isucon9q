...
func getUserID(r *http.Request) (userID int64, errCode int, errMsg string) {
	session := getSession(r)
	vid, ok := session.Values["user_id"]
	if !ok {
		return 0, http.StatusNotFound, "no session"
	}
	return vid.(int64), http.StatusOK, ""
}
...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	inQuery, inArgs, err = sqlx.In(`SELECT
	items.id,
	items.seller_id,
	items.status,
	items.name,
	...
	c.category_name
FROM items
INNER JOIN users AS seller ON items.seller_id = seller.id
INNER JOIN categories AS c ON c.id = items.category_id
WHERE items.id IN (?)
ORDER BY items.created_at DESC, items.id DESC`, itemIDs)
	if err != nil {
		outputErrorMsg(w, http.StatusNotFound, "failed to select users: "+err.Error())
		return
	}
	err = dbx.Select(&bindItems, inQuery, inArgs...)
	if err != nil {
		outputErrorMsg(w, http.StatusNotFound, "failed to select users: "+err.Error())
		return
	}
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		inQuery, inArgs, err = sqlx.In(
			"SELECT id FROM `items` WHERE `status` IN (?,?) AND category_id IN (?) AND (`created_at` < ?  OR (`created_at` <= ? AND `id` < ?)) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			categoryIDs,
			time.Unix(createdAt, 0),
		...
		// 1st page
		inQuery, inArgs, err = sqlx.In(
			"SELECT id FROM `items` WHERE `status` IN (?,?) AND category_id IN (?) ORDER BY created_at DESC, id DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			categoryIDs,
			ItemsPerPage+1,
	...
	var itemIDs []int64
	err = dbx.Select(&itemIDs, inQuery, inArgs...)
